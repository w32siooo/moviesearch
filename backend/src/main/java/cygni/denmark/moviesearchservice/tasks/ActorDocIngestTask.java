...
public class ActorDocIngestTask {
    ...
    public void streamToElasticFromJpaAndBlock() {
        Flux.using(() -> jdbcTemplate.queryForStream(postgresToElasticQuery, (resultSet, rowNum) -> new ActorDb(
    ...
    private Mono<Long> directIndexActorsFlux(Flux<ActorDb> actorsFlux) {
        ...
        return actorsFlux.map(actorDb -> modelMapper.map(actorDb, ActorDocument.class))
                .window(elasticWindowSize) // Splits flux into multiple windows so elastic doesn't choke.
