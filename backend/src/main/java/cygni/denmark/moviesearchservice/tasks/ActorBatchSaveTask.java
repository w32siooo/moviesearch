...
""";
  ...
  private void flattenInnerListsandEnrich(
      List<ActorDb> actors,
      List<AbstractIngestTask.StringUUID> knownFor,
      List<AbstractIngestTask.StringUUID> primaryProf) {
    actors.forEach(
        actor -> {
          actor
              .getKnownForTitles()
              .forEach(
                  known -> knownFor.add(new AbstractIngestTask.StringUUID(actor.getId(), known)));
          actor
